...
const Books = () => {
    ...
    const updatePageTitle = async () => {
        if (formatParam === 'physical') {
            ...
            if (college) {
                if (selectedCategory !== 'all') {
                    setPageTitle(`${college.name} - ${selectedCategory}`);
                } else {
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div>
                            <h1 className="text-3xl font-bold text-gray-900 dark:text-slate-100 flex items-center">
                                ...
                                {selectedCategory === 'Basic Science & Humanities' && <FlaskConical className="mr-3 text-indigo-600" size={32} />}
                                {pageTitle}
                            </h1>
                        ...
                        {colleges.map(college => (
                            <button
                                ...
                                onClick={() => {
                                    setSearchParams(prev => {
                                        const newParams = new URLSearchParams(prev);
                                        newParams.set('college', college.id);
                                        newParams.delete('category');
                                        return newParams;
                                    });
                                }}
                            ...
                            .map(dept => (
                                <button
                                    ...
                                    onClick={() => {
                                        setSearchParams(prev => {
                                            const newParams = new URLSearchParams(prev);
                                            newParams.set('category', dept.name);
                                            return newParams;
                                        });
                                    }}
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
                        {filteredBooks.map((book) => (
                            <motion.div
                                ...
                                initial={{ opacity: 0, y: 20 }}
                                animate={{ opacity: 1, y: 0 }}
                                className="bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden group dark:bg-white/5 dark:backdrop-blur-xl dark:border dark:border-white/10 dark:shadow-[0_8px_32px_0_rgba(0,0,0,0.37)] dark:hover:bg-white/10 dark:hover:shadow-[0_0_20px_rgba(225,29,72,0.2)] flex flex-col"
                            >
                                <div className="aspect-[3/4] relative overflow-hidden bg-gray-100 dark:bg-slate-800 flex items-center justify-center">
                                    {book.coverPath ? (
                                        <img
